...
| **CLI** | Typer + Rich || **Settings** | pydantic-settings (env-driven, type-safe config) |
| **Logging** | structlog (JSON or console, Loki-ready) |
| **Type checking** | Pyrefly (static analysis, pre-commit) |
| **Toolchain** | uv (dependency management, lockfile) || **Security** | OWASP headers, SCRAM-SHA-256, read-only containers, Trivy CI scanning |
