---
title: Installation
description: Complete guide to installing GreenKube via Helm or from source.
---
...
2. **Create a values file**
   ...
   config:
     cloudProvider: aws  # aws, gcp, azure, ovh, scaleway, on-prem
...
| `ClusterRole` | Read access to nodes, pods, namespaces, endpoints, persistentvolumes, persistentvolumeclaims |
...
| `ServiceMonitor` | Prometheus Operator scrape config (if monitoring enabled) |
| `NetworkPolicy` | Allows Prometheus to reach GreenKube API (if monitoring enabled) |
