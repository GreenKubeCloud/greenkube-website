...
| `config.api.apiKey` | Bearer token for API authentication (empty = no auth) | `""` |
| `config.api.corsOrigins` | Allowed CORS origins (comma-separated) | `*` |
| `config.api.rateLimit` | Rate limit (requests per minute, 0 = disabled) | `0` |
