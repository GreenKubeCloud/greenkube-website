...
<Aside type="caution">
  **Current scope:** Energy and CO₂e estimation is currently based on **CPU usage only**. Memory, network I/O, disk I/O, and GPU are collected as metrics but are **not yet factored into the energy model**. GPU workloads are not supported. These dimensions are planned for a future release.
</Aside>
...
GreenKube uses the **Electricity Maps API** for real-time intensity data by default. Since v0.3.0, the free EU-funded **[Wattnet](https://wattnet.eu)** API can be selected instead (`config.electricityProvider: wattnet`) for 15-minute-resolution data across 52 European zones — see the [Wattnet guide](/guide/wattnet/). When neither provider's API is reachable, GreenKube follows a two-step fallback:
...
| Provider | PUE | Source |
|----------|-----|--------|
| AWS | 1.15 | [AWS Sustainability](https://sustainability.aboutamazon.com/products-services/aws-cloud) |
| GCP | 1.09 | [Google Data Centers](https://datacenters.google/efficiency/) |
| Azure | 1.18 | [Microsoft Sustainability Report 2025](https://cdn-dynmedia-1.microsoft.com/is/content/microsoftcorp/microsoft/msc/documents/presentations/CSR/2025-Microsoft-Environmental-Sustainability-Report.pdf) |
| OVH | 1.26 | [OVHcloud Sustainability](https://corporate.ovhcloud.com/en/sustainability/environment/) |
| Scaleway | 1.37 | [Scaleway Environmental Leadership](https://www.scaleway.com/fr/leadership-environnemental/) |
...
| Electricity Maps | Grid carbon intensity (default provider) | Cached (hourly default) |
| Wattnet | Grid carbon intensity (EU-only, alternative provider) | Cached (15-minute resolution) |
...
- [Wattnet API](https://api.wattnet.eu/v1/docs)
