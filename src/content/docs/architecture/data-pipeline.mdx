...
The `DataProcessor` runs collection in **four explicit phases** to prevent race conditions and ensure deterministic results:

| Phase | Description | Execution |
|-------|-------------|-----------|
| **Phase 1** | Node discovery (Kubernetes API) | Sequential — must complete before anything else |
| **Phase 2** | Zone resolution (cloud provider → Electricity Maps zone) | Sequential |
| **Phase 3** | Metrics + Boavizta collection | Concurrent (`asyncio.gather`) |
| **Phase 4** | Carbon-intensity prefetch + metric assembly | Sequential |
...
The `DataProcessor` orchestrates the transformation pipeline by delegating to focused collaborators. Each collaborator handles a single responsibility.

### Step 1: Resource Aggregation (`PrometheusResourceMapper`)
...
### Step 2: Energy Estimation (`BasicEstimator`)
...
### Step 3: Zone Mapping (`NodeZoneMapper`)
...
### Step 4b: Cost Normalization (`CostNormalizer`)

Divides OpenCost daily/range cost totals into per-step values to ensure cost data is proportional to each collection interval rather than the full reporting period.

### Step 5: Carbon Calculation (`CarbonCalculator`)
...
### Step 6: CombinedMetric Assembly (`MetricAssembler`)
...
For reporting on past time periods, the `HistoricalRangeProcessor` uses a **chunked processing** approach:
